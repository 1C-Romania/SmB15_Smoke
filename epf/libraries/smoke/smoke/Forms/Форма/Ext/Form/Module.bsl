...
Процедура ЯОткрываюФормуСправочникаДляСуществующегоЭлементаСоСвойствомИЗначением(Мета_ПолноеИмя, ИмяСвойства="", Значение="") Экспорт
	//Ванесса.ПосмотретьЗначение(Парам01,Истина);
	ПараметрыФормы = Новый Структура("Ключ", ТестДолжен_ОткрытьФормуСуществующегоЭлементаСправочникаСервер(Мета_ПолноеИмя));
...
Процедура ЯОткрываюФормуДокументаДляСуществующегоЭлементаСоСвойствомИЗначением(Мета_ПолноеИмя, ИмяСвойства="", Значение="") Экспорт
	...
	ПараметрыФормы = Новый Структура("Ключ", CоздаваемыйЭлемент);
	Если НЕ ПустаяСтрока(ИмяСвойства) Тогда 
		ПараметрыФормы.Вставить(ИмяСвойства, Значение);
	КонецЕсли;
		...
		Для каждого Элемент Из Состав Цикл
			...
			Если ЭтоСправочник Тогда
				...
				Если Элемент.ОсновнаяФормаОбъекта <> Неопределено Тогда
					...
					Текст.ДобавитьСтроку("		Когда Я открываю форму справочника '"+Форма.ПолноеИмя()+"' для нового элемента" + " со свойством 'AutoTest' и значением '1'");
					Текст.ДобавитьСтроку("		И Я открываю форму справочника '"+Форма.ПолноеИмя()+"'  для существующего элемента" + " со свойством 'AutoTest' и значением '1'");					
				КонецЕсли; 
				
				Для каждого Форма Из Элемент.Формы Цикл
					Если МассивФорм.Найти(Форма) = Неопределено Тогда
						Текст.ДобавитьСтроку("		И	Я открываю произвольную форму '"+Форма.ПолноеИмя()+"'" + " со свойством 'AutoTest' и значением '1'");						
					КонецЕсли; 
				КонецЦикла;
			ИначеЕсли Метаданные.Документы.Содержит(Элемент) Тогда 
				...
				Если Элемент.ОсновнаяФормаОбъекта <> Неопределено Тогда
					МассивФорм.Добавить(Элемент.ОсновнаяФормаОбъекта);
					Форма = Элемент.ОсновнаяФормаОбъекта; 
					Текст.ДобавитьСтроку("		Когда Я открываю форму документа '"+Форма.ПолноеИмя()+"' для нового элемента" + " со свойством 'AutoTest' и значением '1'");
					Текст.ДобавитьСтроку("		И Я открываю форму документа '"+Форма.ПолноеИмя()+"'  для существующего элемента" + " со свойством 'AutoTest' и значением '1'");					
				КонецЕсли; 
